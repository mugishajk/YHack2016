...
function collect_params(interval, total_news) {
    ...
    //create a list of news index, which goes from 0 - 29 because we only have 29 news stored in our database
    //if we random chose 8 news, then there will be 8 news_indexes
    var news_index = shuffle(generateUniqueRandom(29), total_news);
...
/*
    //generate unique random integers between 0 - 29
    console.log("total news: " + total_news);
    console.log("print news index " + i + ": " + )
    for (var i=0; i < total_news.length; i++) {
        var random_news_index = shuffle(generate)
        news_index.push(shuffle(generateUniqueRandom(29)));
    }
*/ 
